...
case class GpuSha2(left: Expression, right: Expression)
  ...
  override def doColumnar(lhs: GpuColumnVector, rhs: GpuScalar): ColumnVector = {
    if (bitLength == null) {
      ...
      val nullStringExemplar = Scalar.fromNull(DType.STRING)
      ColumnVector.fromScalar(nullStringExemplar, lhs.getRowCount.toInt)
        ...
        bitLength match {
          ...
          case _ => throw new UnsupportedOperationException("CALEB: Not implemented yet")
            /*
          case 256 =>
            withResource(ColumnVector.sha256Hash(normalized)) { fullResult =>
              fullResult.mergeAndSetValidity(BinaryOp.BITWISE_AND, normalized)
            }
          case 384 =>
            withResource(ColumnVector.sha384Hash(normalized)) { fullResult =>
              fullResult.mergeAndSetValidity(BinaryOp.BITWISE_AND, normalized)
            }
          case 512 =>
            withResource(ColumnVector.sha512Hash(normalized)) { fullResult =>
              fullResult.mergeAndSetValidity(BinaryOp.BITWISE_AND, normalized)
            }
             */
        }
