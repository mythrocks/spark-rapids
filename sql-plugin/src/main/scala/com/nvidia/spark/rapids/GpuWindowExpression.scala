...
abstract class GpuSpecifiedWindowFrameMetaBase(
  ...
  /*
  def checkBounds2[T <: Ordered[T]](boundsValue: T, isLower: Boolean): Option[String] = {
    if (isLower && boundsValue > 0.asInstanceOf[T]) {
      Some(s"Lower-bounds ahead of current row is not supported. Found: $boundsValue")
    }
    else if (!isLower && boundsValue < 0.asInstanceOf[T]) {
  ...
  private def checkBounds(boundsValue: BigInt, isLower: Boolean): Option[String] = {
    if (isLower && boundsValue > 0) {
      Some(s"Lower-bounds ahead of current row is not supported. Found: $boundsValue")
    }
    else if (!isLower && boundsValue < 0) {
      Some(s"Upper-bounds behind current row is not supported. Found: $boundsValue")
    }
    else {
      None
  ...
  private def checkBounds(boundsValue: Double, isLower: Boolean): Option[String] = {
    if (isLower && boundsValue > 0) {
      Some(s"Lower-bounds ahead of current row is not supported. Found: $boundsValue")
    }
    else if (!isLower && boundsValue < 0) {
  ...
  override def tagExprForGpu(): Unit = {
    if (windowFrame.frameType.equals(RangeFrame)) {
      ...
      def checkIfInvalid(bounds : Expression, isLower : Boolean) : Option[String] = {
         ...
         *  1. lower-bound can't be ahead of the current row.
         *  2. upper-bound can't be behind the current row.
         */
